...
export default function FollowersPage() {
  ...
  useEffect(() => {
    setPage(1);
    setHasMore(true);
    void fetchFollowers(1, debouncedSearch, true);
  ...
  const handleLoadMore = useCallback(() => {
    ...
    setPage(nextPage);
    void fetchFollowers(nextPage, debouncedSearch, false);
  }, [
    debouncedSearch,
    fetchFollowers,
    hasMore,
    ...
    async (followerId: string) => {
      try {
        ...
        if (response.data.success) {
          setFollowers((current) =>
            current.filter((follower) => follower._id !== followerId),
          );
          toast({
            variant: "success",
            title: "Success",
        ...
        toast({
          variant: "error",
          title: "Error",
          description: message || "Could not remove follower.",
        });
      }
    },
    [toast],
  ...
  const renderFollower = useCallback(
    ({ item }: { item: FollowerUser }) => (
      <View style={[styles.row, { borderColor: border }]}>
        ...
        {isOwner ? (
          <Pressable
            onPress={() => void handleRemoveFollower(item._id)}
            style={({ pressed }) => [
              styles.removeButton,
            ...
            accessibilityRole="button"
            accessibilityLabel={`Remove ${item.username || "follower"}`}
          >
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: background }]}>
      <View style={[styles.page, { backgroundColor: background }]}>
        ...
        >
          ...
          <Text style={[styles.headerTitle, { color: textPrimary }]}>Followers</Text>
          ...
          <FlatList
            ...
            ListEmptyComponent={
              ...
              >
                ...
                <Text style={[styles.emptyTitle, { color: textPrimary }]}>No followers found</Text>
...
const styles = StyleSheet.create({
  ...
  removeButton: { minHeight: 34, paddingHorizontal: 14, borderRadius: 10, alignItems: "center", justifyContent: "center" },
